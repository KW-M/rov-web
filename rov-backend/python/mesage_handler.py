...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    async def handle_rov_status_report(self, src_peer_id: str, msg_data: RovAction) -> tuple[AsyncGenerator, list[str]]:
        ...
        msg_generator = generate_cmd_continued_output_response(msg_data.rov_exchange_id, "/home/pi/rov-web/rasberry_pi_setup_scripts/rov_status_report.sh", cmd_timeout=20)
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    async def handle_restart_rov_services(self, src_peer_id: str, msg_data: RovAction) -> tuple[AsyncGenerator, list[str]]:
        msg_generator = generate_cmd_continued_output_response(msg_data.rov_exchange_id, "/home/pi/rov-web/rasberry_pi_setup_scripts/fetch_changes.sh", cmd_timeout=20)
        return (msg_generator, [src_peer_id])
