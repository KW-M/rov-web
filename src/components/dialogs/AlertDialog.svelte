<script>
  ...
  function onClose(ok_clicked) {
    dispatch("close", ok_clicked);
  }
</script>

<Dialog on:close={() => onClose(false)} modalStyle={"max-width:16rem; text-align:center"} modalClass="modal-bottom">
