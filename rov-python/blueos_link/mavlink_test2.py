...
async def send_heartbeat_loop(base_mode=192, custom_mode=0, system_status=mavutil.mavlink.MAV_STATE_STANDBY):
    while True:
        try:
...
master = mavutil.mavlink_connection('udpout:0.0.0.0:8235')
...
async def main():
    await asyncio.gather(get_mavlink_msgs(),send_heartbeat_loop())
