...
master: mavutil.mavudp = mavutil.mavlink_connection('udpout:0.0.0.0:8235',source_system=171,autoreconnect=True)
...
async def send_heartbeats():
    ...
    while True:
        try:
            ...
            # master.mav.command_long_send(1,0,mavutil.mavlink.MAV_CMD_COMPONENT_ARM_DISARM,0,1,0,0,0,0,0,0) # 0 = normal arm
            # master.mav.manual_control_send(1,1000,-1000,0,0,0)
        except Exception as e:
            print(e)
            pass
...
async def send_control():
    ...
    # master.mav.command_long_send(1,0,mavlink2.MAV_CMD_COMPONENT_ARM_DISARM,0,1,21196,0,0,0,0,0)
    master.mav.command_long_send(1,0,mavlink2.MAV_CMD_COMPONENT_ARM_DISARM,0,1,0,0,0,0,0,0)
    print("MAV arm sent---------------")
    await asyncio.sleep(1)
    master.mav.set_mode_send(1,mavlink2.MAV_MODE_MANUAL_ARMED,0)
    print("MAV mode sent---------------")
    counter = 0
    while True:
        counter += 1
        try:
            # if counter % 6 == 0:
            #     master.mav.manual_control_send(1,random.randrange(1,2) * 5,0,0,0,0)
            # else:
            #     master.mav.manual_control_send(1,0,0,0,0,0)
            master.mav.manual_control_send(1,900,-1000,8,0,0)
            # master.mav.rc_channels_override_send(1,1,num,num*2,num*3,num*4,num*5,num*6,num*8,num*9)
        except Exception as e:
            print(e)
            pass
